...
function computeChecksum(importantData) {
	...
	for (i=0; i<importantData.length; i++) {
		cs += importantData.charCodeAt(i) * (i+1);
	}
	return cs;
}
...
function corruptDataRandomly(importantData) {
	...
	//ADD THIS
	   ...
	   return newFunction(importantData);
	}
...
//AND THIS
function newFunction(importantData){
	var up = String.fromCharCode( importantData.charCodeAt(1) + 1 );
	var upagain = String.fromCharCode( importantData.charCodeAt(3) + 1 );
	var down = String.fromCharCode( importantData.charCodeAt(5) - 1 );
	var str = importantData.substr(0,1) + up + importantData.substr(2,1) + upagain + importantData.substr(4,1) + down + importantData.substr(6);
	return str;
}
